...
class _AdminStatisticsScreenState extends State<AdminStatisticsScreen> {
  ...
  String _getUserName(Map<String, dynamic> data) {
    // 1. Kiểm tra các trường tên hiển thị có sẵn trong bản ghi history
    final name = data['user_name'] ?? data['userName'] ?? data['username'];
    if (name != null && name.toString().trim().isNotEmpty) {
      return name.toString();
    }

    // 2. Nếu không có tên, kiểm tra ID người dùng (UID)
    final userId = data['user_id']?.toString() ?? '';
    if (userId.isNotEmpty) {
      if (userId.length > 10) {
        return "Khách (...${userId.substring(userId.length - 5)})";
      }
      return "Khách ($userId)";
    }
  ...
  Future<void> _exportToExcel(List<QueryDocumentSnapshot> docs, double totalRevenue, double totalDeposit, int soldAccCount) async {
    try {
      ...
      var titleStyle = ex.CellStyle(bold: true, fontSize: 16, horizontalAlign: ex.HorizontalAlign.Center);
      var headerStyle = ex.CellStyle(bold: true, backgroundColorHex: ex.ExcelColor.fromHexString('#D3D3D3'), horizontalAlign: ex.HorizontalAlign.Center, verticalAlign: ex.VerticalAlign.Center, leftBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), rightBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), topBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), bottomBorder: ex.Border(borderStyle: ex.BorderStyle.Thin));
      var cellStyle = ex.CellStyle(horizontalAlign: ex.HorizontalAlign.Left, verticalAlign: ex.VerticalAlign.Center, leftBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), rightBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), topBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), bottomBorder: ex.Border(borderStyle: ex.BorderStyle.Thin));
      var cellCenterStyle = ex.CellStyle(horizontalAlign: ex.HorizontalAlign.Center, verticalAlign: ex.VerticalAlign.Center, leftBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), rightBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), topBorder: ex.Border(borderStyle: ex.BorderStyle.Thin), bottomBorder: ex.Border(borderStyle: ex.BorderStyle.Thin));
      ...
      sheet.merge(ex.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: rowIndex), ex.CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: rowIndex));
      rowIndex += 2;
      ...
      for (int i = 0; i < docs.length; i++) {
        ...
        final user = _getUserName(data);
        final amount = _asDouble(data['amount']).toInt();
        final createdAt = (data['created_at'] as Timestamp?)?.toDate() ?? now;
        final timeStr = DateFormat('dd/MM/yyyy HH:mm').format(createdAt);
        String detail = isPurchase ? "Mã acc: #${data['account_code'] ?? 'N/A'}" : (data['method'] == 'card' ? 'Nạp thẻ cào' : 'Nạp ví ATM/Momo');
        List<ex.CellValue> rowData = [ex.IntCellValue(i + 1), ex.TextCellValue(type), ex.TextCellValue(user), ex.IntCellValue(amount), ex.TextCellValue(detail), ex.TextCellValue(timeStr)];
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi xuất báo cáo: $e'), backgroundColor: Colors.redAccent));
    }
  }
        ...
        child: Column(
          children: [
            const Padding(
              ...
              child: Center(
                child: Text('BÁO CÁO & THỐNG KÊ HỆ THỐNG', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 26, letterSpacing: 2)),
              ),
            ),
            _buildFilterBar(),
            Container(
          ...
          children: StatFilter.values.map((filter) {
            ...
            return Padding(
              ...
              child: ChoiceChip(
                ...
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: isSelected ? AppStyles.primaryColor : Colors.white24)),
              ),
            );
          }).toList(),
        ),
  ...
  Widget _buildRevenueSection() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        double totalRevenue = 0; double totalDeposit = 0; int soldAccCount = 0; double cardRev = 0; double atmRev = 0;
        Map<int, double> monthlyRev = {}; final now = DateTime.now();
        ...
        for (var doc in filteredDocs) {
          ...
          final date = (data['created_at'] as Timestamp?)?.toDate() ?? now;
          if (type == 'purchase') {
            totalRevenue += amount; soldAccCount++;
            if (date.year == now.year) monthlyRev[date.month] = (monthlyRev[date.month] ?? 0) + amount;
          } else if (type == 'deposit') {
            totalDeposit += amount;
            if (data['method'] == 'card') cardRev += amount; else atmRev += amount;
          }
        }
        return Column(
          children: [
            ...
            Padding(
              ...
              child: ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade600, foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))),
              ),
            ),
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                return Flex(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Expanded(flex: isWide ? 3 : 0, child: GlassContainer(padding: const EdgeInsets.all(25), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Biến động doanh thu', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 30), SizedBox(height: 200, child: _SimpleLineChart(monthlyData: monthlyRev))]))),
                    if (isWide) const SizedBox(width: 20) else const SizedBox(height: 20),
                    Expanded(flex: isWide ? 2 : 0, child: GlassContainer(padding: const EdgeInsets.all(25), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Nguồn nạp tiền', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 30), _buildPaymentProgress('Thẻ cào', totalDeposit > 0 ? cardRev / totalDeposit : 0, Colors.orange), const SizedBox(height: 25), _buildPaymentProgress('ATM / Momo', totalDeposit > 0 ? atmRev / totalDeposit : 0, Colors.purpleAccent)]))),
                  ],
                );
              }
            ),
  ...
  Widget _buildRecentTransactions() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (filteredDocs.isEmpty) return const GlassContainer(padding: EdgeInsets.all(40), child: Center(child: Text("Không có giao dịch", style: TextStyle(color: Colors.white38))));
        ...
        return GlassContainer(
          ...
          child: Column(
            children: filteredDocs.map((doc) {
              ...
              return ListTile(
                leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: (isPurchase ? Colors.red : Colors.green).withOpacity(0.15), borderRadius: BorderRadius.circular(12)), child: Icon(isPurchase ? Icons.shopping_bag : Icons.account_balance_wallet, color: isPurchase ? Colors.redAccent : Colors.greenAccent, size: 24)),
                title: uid.isEmpty 
                  ? Text(_getUserName(data), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))
                  : FutureBuilder<DocumentSnapshot>(
                      future: _firestore.collection('user').doc(uid).get(),
                      builder: (context, userSnap) {
                        String displayName = _getUserName(data);
                        if (userSnap.hasData && userSnap.data!.exists) {
                          final uData = userSnap.data!.data() as Map<String, dynamic>?;
                          displayName = uData?['user_name'] ?? uData?['userName'] ?? uData?['username'] ?? displayName;
                        }
                        return Text(displayName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
                      },
                    ),
                subtitle: Text(DateFormat('dd/MM HH:mm').format(createdAt), style: const TextStyle(color: Colors.white54)),
                trailing: Text('${isPurchase ? "-" : "+"}${currencyFormat.format(_asDouble(data['amount']))}', style: TextStyle(color: isPurchase ? Colors.redAccent : Colors.greenAccent, fontWeight: FontWeight.w900)),
              );
            }).toList(),
          ),
        );
  ...
  Widget _buildProductSection() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
        final now = DateTime.now(); final docs = snapshot.data!.docs;
        int totalCount = docs.length; int soldCount = 0; int oldStockCount = 0; Map<String, int> rankStats = {};
        for (var doc in docs) {
          final data = doc.data() as Map<String, dynamic>;
          ...
          if (isSold) { soldCount++; rankStats[rank] = (rankStats[rank] ?? 0) + 1; }
          else { if (now.difference(createdAt).inDays > 30) oldStockCount++; }
        }
        String bestSellingRank = rankStats.isNotEmpty ? rankStats.entries.reduce((a, b) => a.value > b.value ? a : b).key : "Chưa có";
        double sellRate = totalCount > 0 ? (soldCount / totalCount * 100) : 0;
        return Wrap(
          spacing: 20, runSpacing: 20,
          children: [
            _buildStatCard('TỔNG TÀI KHOẢN', totalCount.toString(), Colors.cyan, Icons.storage, onTap: () => Navigator.pushNamed(context, '/admin-accounts')),
  ...
  Widget _buildUserSection() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        double systemDebtValue = snapshot.data!.docs.fold(0.0, (acc, doc) => acc + _asDouble((doc.data() as Map)['balance']));
        return Wrap(
          spacing: 20, runSpacing: 20,
          children: [
            _buildStatCard('THÀNH VIÊN', totalUsers.toString(), Colors.indigoAccent, Icons.person_add, onTap: () => Navigator.pushNamed(context, '/admin-users')),
  ...
  Widget _buildStatCard(String title, String value, Color color, IconData icon, {double width = 275, bool isFullWidth = false, VoidCallback? onTap}) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Material(color: Colors.transparent, child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(24), child: Container(width: effectiveWidth, padding: const EdgeInsets.all(25), decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(24), border: Border.all(color: color.withOpacity(0.4), width: 1.5), boxShadow: [BoxShadow(color: color.withOpacity(0.05), blurRadius: 15)]), child: Row(children: [Icon(icon, color: color, size: 32), const SizedBox(width: 20), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Text(title, style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 13, fontWeight: FontWeight.bold)), if (onTap != null) ...[const SizedBox(width: 5), Icon(Icons.arrow_forward_ios, size: 10, color: Colors.white.withOpacity(0.3))]]), const SizedBox(height: 5), Text(value, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900), overflow: TextOverflow.ellipsis)]))]))));
    });
  }

  Widget _buildPaymentProgress(String label, double percent, Color color) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.w600)), Text('${(percent * 100).toStringAsFixed(1)}%', style: TextStyle(color: color, fontWeight: FontWeight.bold))]), const SizedBox(height: 12), ClipRRect(borderRadius: BorderRadius.circular(10), child: LinearProgressIndicator(value: percent, minHeight: 10, backgroundColor: Colors.white12, valueColor: AlwaysStoppedAnimation<Color>(color)))]);
  }
}
...
class _SimpleLineChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { return CustomPaint(size: Size.infinite, painter: _LineChartPainter(monthlyData)); }
}
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double leftMargin = 45.0; double bottomMargin = 25.0; double chartWidth = size.width - leftMargin; double chartHeight = size.height - bottomMargin;
    ...
    double maxVal = monthlyData.values.fold(0, (max, v) => v > max ? v : max); if (maxVal == 0) maxVal = 1;
    ...
    final points = <Offset>[]; final stepX = chartWidth / 11;
    for (int i = 1; i <= 12; i++) { double val = monthlyData[i] ?? 0; points.add(Offset(leftMargin + (i - 1) * stepX, chartHeight - (val / maxVal * chartHeight * 0.8))); }
    ...
    for (var p in points) { canvas.drawCircle(p, 4, dotPaint); canvas.drawCircle(p, 2, paint); }
    for (int i = 1; i <= 12; i += 2) { textPainter.text = TextSpan(text: 'T$i', style: const TextStyle(color: Colors.white38, fontSize: 10)); textPainter.layout(); textPainter.paint(canvas, Offset(leftMargin + (i - 1) * stepX - 5, chartHeight + 5)); }
  }
  @override
